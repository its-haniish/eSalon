...
const Contact = ({setPage}) => {
    ...
    useEffect(() => {
        const showUserInfo = async () => {
                let response = await fetch('https://esalon-server.onrender.com/getShop', {
                    method: 'POST',
                    body: JSON.stringify({ email: JSON.parse(localStorage.getItem('eSalon-Shop')).email }),
  ...
  return (<>
        ...
        <form className="contact-form" onSubmit={handleSendMessage}>
            ...
            type="text" 
            name='shopName' 
            value={data?.shopName || 'Error'} />
            <input 
            type="email" 
            name='email' 
            value={JSON.parse(localStorage.getItem('eSalon-Shop')).email} />
            <input 
            type="number" 
            name='phone' 
